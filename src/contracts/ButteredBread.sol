...
import {IButteredBread} from '../interfaces/IButteredBread.sol';
...
contract ButteredBread is IButteredBread, ERC20VotesUpgradeable, OwnableUpgradeable {
  ...
  function initialize(InitData calldata _initData) external initializer {
    ...
    emit InitializedEvent(_initData.name, _initData.symbol, _initData.breadToken);
  }
  ...
  function _syncVotingWeight(address _account, address _lp) internal {
    ...
    if (currentScalingFactor != initialScalingFactor) {
      ...
      if (lpBalance > 0) {
        if (currentScalingFactor > initialScalingFactor) {
          _mint(_account, (lpBalance * currentScalingFactor - lpBalance * initialScalingFactor) / FIXED_POINT_PERCENT);
        } else {
          _burn(_account, (lpBalance * initialScalingFactor - lpBalance * currentScalingFactor) / FIXED_POINT_PERCENT);
        }
      }
    }
  }
